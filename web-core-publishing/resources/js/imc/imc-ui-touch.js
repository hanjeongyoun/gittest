...
$(function(){
	...
	window.POP_TOUCH = (function(){
		...
		_bindEvent = function(){
			$(document).on("click", "input[type='text']:not([data-nokeypad='true']) , input[type='number']:not([data-nokeypad='true'])", function(e){
				$callBack = $(e.target);
				show($callBack.val(), notifyChange, $callBack.data("valid") ? $callBack.data("valid") : "txt");
			});
		...
		hide = function(){
			if(!Dialog.isDialog()){
				$keypad.removeClass("txt number float")
			}else{
				top.POP_TOUCH.hide();
			}
		},
		show = function(val, notifyChange, keypadType){
			if(!Dialog.isDialog()){
				$keypad.removeClass("txt number float").addClass(keypadType).find("input").val(val);
				callbackFn = notifyChange;
			}else{
				top.POP_TOUCH.show(val, notifyChange, keypadType);
			}
		};
